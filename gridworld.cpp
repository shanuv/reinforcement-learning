	...
	while(trial<trials){
		...
		for( int count =0; count < episodes; count++){
			...
			while(!(curr_i == 4 && curr_j == 4)){
				...
				//cout << curr_state << " " << calc_state << endl;
				...
				weights_epsilongreedy[max_index]+=(1-epsilon);
				discrete_distribution<> distr1(weights_epsilongreedy, weights_epsilongreedy + sizeof weights_epsilongreedy/sizeof weights_epsilongreedy[0]);
				calc_action = distr1(generator);
				weights_epsilongreedy[max_index]-=(1-epsilon);
				for(int i=0;i<23;i++)
					for (int j=0;j<4;j++)
				...
				for(int i=0;i<23;i++)
					for (int j=0;j<4;j++)
						actionvalue[i][j]+=alpha*(grid[calc_i][calc_j]+gamma*actionvalue[calc_state-1][calc_action]-actionvalue[curr_state-1][action])*eligibility[i][j];

				curr_i = calc_i;					//New State
				curr_j = calc_j;

				action = calc_action;//_taken;			//New action
