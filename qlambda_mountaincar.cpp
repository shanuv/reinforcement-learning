/*Implementation of the Q-lambda algorithm for the Mountaincar domain using the Fourier basis
	...
	while(trial<trials){
		...
		for( int count =0; count < N; count++){
			...
			while(new_x!=0.5){
				...
				for (int j=0;j<(int)pow(basis+1,dim);j++)
					for(int k=0;k<3;k++)
						weights(j,k) = weights(j,k)+(alpha*(-1+gamma*max-value_qsa(action))*eligibility(j,k)); //Sarsa update
